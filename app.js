(function(){
  ...
  var ACCEPT_EXT = { log:1, txt:1, out:1, err:1, trace:1, syslog:1 };
  ...
  /* buildRendered() provided by the Log adapter above */
  ...
  // ---------- View orchestration ----------
  var LINEH = 20, OVER = 8;
  var logLines = [], logLevel = [], logRowsIdx = [], logQ = "";
  var logActive = { ERROR: true, WARN: true, INFO: true, DEBUG: true, OTHER: true };
  var logScroll, logRowsEl, logViewport, logCountEl, logSearchEl;

  function fmtNum(n){ return String(n).replace(/\B(?=(\d{3})+(?!\d))/g, ","); }
  function detectLevel(line){
    var m = /\b(FATAL|CRITICAL|CRIT|SEVERE|ERROR|ERR|WARN(?:ING)?|NOTICE|INFO|DEBUG|TRACE|VERBOSE)\b/.exec(line);
    if (!m) m = /\b(fatal|error|warn(?:ing)?|info|debug|trace)\b/.exec(line);
    if (!m) return "OTHER";
    var t = m[1].toUpperCase();
    if (t === "FATAL" || t === "CRITICAL" || t === "CRIT" || t === "SEVERE" || t === "ERR") return "ERROR";
    if (t.indexOf("WARN") === 0) return "WARN";
    if (t === "NOTICE") return "INFO";
    if (t === "TRACE" || t === "VERBOSE") return "DEBUG";
    return t === "ERROR" || t === "INFO" || t === "DEBUG" || t === "WARN" ? t : "OTHER";
  }
  ...
  function show(text, name){
    currentName = name || ""; rawText = String(text || ""); hasRendered = true; parseErr = "";
    syncQueryName(currentName);
    logLines = rawText.split("\n");
    if (logLines.length && logLines[logLines.length - 1] === "") logLines.pop();
    logLevel = new Array(logLines.length);
    var counts = { ERROR: 0, WARN: 0, INFO: 0, DEBUG: 0, OTHER: 0 };
    for (var i = 0; i < logLines.length; i++){ var lv = detectLevel(logLines[i]); logLevel[i] = lv; counts[lv]++; }
    logActive = { ERROR: true, WARN: true, INFO: true, DEBUG: true, OTHER: true }; logQ = "";
    empty.hidden = true;
  ...
  function buildLogUI(counts){
    dataView.innerHTML = ""; dataView.className = "dataview log";
    var bar = el("div", "log-bar");
    logSearchEl = doc.createElement("input");
    logSearchEl.type = "search"; logSearchEl.className = "log-search";
    logSearchEl.placeholder = "Filter lines…"; logSearchEl.setAttribute("aria-label", "Filter log lines");
    bar.appendChild(logSearchEl);
    var chips = el("div", "log-chips");
    ["ERROR", "WARN", "INFO", "DEBUG", "OTHER"].forEach(function(lv){
      if (!counts[lv]) return;
      var c = doc.createElement("button");
      c.type = "button"; c.className = "log-chip lvl-" + lv + " on"; c.setAttribute("aria-pressed", "true");
      c.textContent = lv + " " + fmtNum(counts[lv]);
      c.addEventListener("click", function(){
        logActive[lv] = !logActive[lv];
        c.classList.toggle("on", logActive[lv]); c.setAttribute("aria-pressed", logActive[lv] ? "true" : "false");
        applyFilter();
      });
      chips.appendChild(c);
    });
    bar.appendChild(chips);
    logCountEl = el("div", "log-count"); bar.appendChild(logCountEl);
    dataView.appendChild(bar);
    logScroll = el("div", "log-scroll");
    logViewport = el("div", "log-viewport");
    logRowsEl = el("div", "log-rows");
    logViewport.appendChild(logRowsEl); logScroll.appendChild(logViewport);
    dataView.appendChild(logScroll);
    logScroll.addEventListener("scroll", renderWindow, { passive: true });
    var t = null;
    logSearchEl.addEventListener("input", function(){ clearTimeout(t); t = setTimeout(function(){ logQ = logSearchEl.value; applyFilter(); }, 110); });
  }

  function applyFilter(){
    var ql = logQ.toLowerCase(); logRowsIdx = [];
    for (var i = 0; i < logLines.length; i++){
      if (!logActive[logLevel[i]]) continue;
      if (ql && logLines[i].toLowerCase().indexOf(ql) < 0) continue;
      logRowsIdx.push(i);
    }
    logViewport.style.height = (logRowsIdx.length * LINEH) + "px";
    logCountEl.textContent = logRowsIdx.length === logLines.length
      ? fmtNum(logLines.length) + " lines"
      : fmtNum(logRowsIdx.length) + " / " + fmtNum(logLines.length);
    logScroll.scrollTop = 0;
    renderWindow();
  }

  function hilite(container, text, term){
    var lt = text.toLowerCase(), lterm = term.toLowerCase(), from = 0, idx;
    while ((idx = lt.indexOf(lterm, from)) >= 0){
      if (idx > from) container.appendChild(doc.createTextNode(text.slice(from, idx)));
      var mk = doc.createElement("mark"); mk.textContent = text.slice(idx, idx + term.length); container.appendChild(mk);
      from = idx + term.length;
    }
    if (from < text.length) container.appendChild(doc.createTextNode(text.slice(from)));
  }
  function renderWindow(){
    if (!logScroll) return;
    var st = logScroll.scrollTop, h = logScroll.clientHeight;
    var first = Math.max(0, Math.floor(st / LINEH) - OVER);
    var last = Math.min(logRowsIdx.length, Math.ceil((st + h) / LINEH) + OVER);
    logRowsEl.style.transform = "translateY(" + (first * LINEH) + "px)";
    var frag = doc.createDocumentFragment();
    for (var i = first; i < last; i++){
      var idx = logRowsIdx[i];
      var row = el("div", "log-row lvl-" + logLevel[idx]);
      var ln = el("span", "log-ln"); ln.textContent = idx + 1;
      var tx = el("span", "log-tx");
      if (logQ) hilite(tx, logLines[idx], logQ); else tx.textContent = logLines[idx];
      row.appendChild(ln); row.appendChild(tx);
      frag.appendChild(row);
    }
    logRowsEl.textContent = ""; logRowsEl.appendChild(frag);
  }

  function buildRendered(){ /* the log UI is built directly in show() */ }
  function readFile(file){
    if (!file) return;
    if (!isAccepted(file.name)){ if (!familyRoute(file)) toast("“" + file.name + "” isn’t a supported log or text file"); return; }
  ...
  /* readFile() provided by the Log adapter above */
  ...
  doc.addEventListener("keydown", function(e){
    if (!id("routeCard").hidden){                 // §6.10: the offer card is aria-modal
      if (e.key === "Escape"){ hideRouteCard(); return; }
      if (e.key === "Tab"){                       // two-button focus trap — Tab and Shift+Tab both wrap
        e.preventDefault();
  ...
  // ---------- Family router (§6.10): map, offer card, sender + hand-off receiver ----------
  // NOTE: .txt is accepted locally (ACCEPT_EXT above) AND owned by markdown in
  // FAMILY_MAP — intentional per §6.10: map ownership decides only where OTHER
  // sites send a rejected file; a .txt dropped here never reaches this code.
  ...
  // Receiver — accept a File handed over from a sibling family tab
  // (fv-ready → fv-file → fv-ack); the clone re-creates a real File here.
  window.addEventListener("message", function(e){
    if (FAMILY_ORIGINS.indexOf(e.origin) === -1) return;      // family origins only
    var d = e.data;
    if (d && d.type === "fv-file" && d.file instanceof File){
      readFile(d.file);
  ...
  if (fvh){
    ...
    if (window.opener){
      ...
      if (emptySub){
        var emptySubPrev = emptySub.textContent;
        emptySub.textContent = "Receiving “⁨" + fvhName + "⁩”…";
        setTimeout(function(){ emptySub.textContent = emptySubPrev; }, 10000);  // hidden once viewing, so an unconditional revert is safe
      }
    }
  }
